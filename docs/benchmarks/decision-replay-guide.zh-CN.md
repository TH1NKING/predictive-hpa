...
既有控制器测试继续检查复用规则之后的真实协调行为；607 条原模式决策匹配与独立核对则提供真实记录的回归证据。Linux 的 `make lint-fix` 与带 race 检查的 `make test`（含独立 envtest）通过；最终分析套件在 Linux 和 Windows 均为 91 项通过。旧实验工具在 Linux 为 82 项通过、3 项 Node 检查由 Windows 补验。审查发现的未来源时间校验缺口也经过红绿测试修复。完整过程、保留的环境失败和验证边界见[验证记录](decision-replay-validation.md)。离线重放仍不能替代新的 Kind 服务实验。
