...
镜像身份还需要区分 **manifest 摘要**和 **config 摘要**。manifest 描述镜像配置与各层，config 描述运行设置及文件系统层身份；Docker 和 CRI 可能分别返回这两种 ID。它们的字符串不同，不代表镜像一定不同。入口先核对构建结果；遇到两种摘要时，再读取已加载的 manifest，验证内容 SHA256，并核对其中引用的 config 与 CRI ID 一致。这样可以建立关联，同时拒绝被替换的内容。直接忽略不相等的 ID 会失去验证作用；直接比较两个不同层级的 ID，则会误拒绝正常镜像。
...
固定镜像仓库不可达时，可以显式使用 `--preload-fixture-image`，将 Docker 中已经存在的同一 digest 镜像加载到新 Kind。它只允许冻结验收配置里声明的引用，在构建和创建集群前检查缓存及摘要，不会改成另一个 tag，也不放宽 Pod 就绪门槛。这样能复现缓存准备路径，并把网络可用性与功能检查分开记录；代价是成功结果不能被描述成完全冷缓存、完全离线的安装。具体命令见[工作流说明](../metrics-safety-workflow.md)。

多架构镜像还会用一个索引引用不同平台的 manifest。本轮实际遇到过：索引包含 AMD64 和 ARM64，但缓存只有 AMD64；Kind 的全平台导入因此拒绝载入。入口改为读取专用节点的实际平台，让 containerd 导入该平台所需内容，并保留原索引与 digest。归档哈希、字节数、节点平台和 CRI 身份都有收据。这能验证当前节点的执行路径，也明确保留了“其他架构尚未验收”的边界。

归档直接以二进制标准输入交给固定节点中的导入进程。实际验收曾遇到文件复制返回成功、导入进程却看不到目标路径的情况；流式输入减少了这项路径依赖。入口在计算输入哈希后将文件位置重置为零，正常退出和异常都会关闭句柄，超时仍终止整个命令进程组。
