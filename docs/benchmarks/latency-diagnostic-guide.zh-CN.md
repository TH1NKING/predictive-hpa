...
使用一个配置开关，让两组运行同一份程序，比为两组分别修改和构建控制器更容易
核对差异。这个设置通过 manager 启动参数传入，没有增加 CRD 字段，因此已有
PHPA 对象不需要迁移。30 秒默认值的回归测试也保留下来，防止诊断能力意外改变
已有用户的行为。

也重新运行了 30 秒组。若只把新 15 秒结果与之前挑出的慢运行比较，机器状态、
采集时机和批次差异都可能被误当作配置收益。因此，先前六次用于发现问题，
新的四次才用于这项配置对照，两批数据不混成一个随机化样本。

15 秒的候选会增加正常协调频率，因此还要看实际查询次数、服务失败和副本占用。
更短的 5 秒或 10 秒会增加更多轮询，本轮没有同时搜索多个值；这能减少事后
挑选最好结果的空间。两个重复和一个特意选中的启动偏移，也不足以决定全局默认值。

四次对照已完成，独立复算 112 项比较全部通过，现场再次恢复。结果如下：

| 每组两次的描述性均值 | 30 秒 | 15 秒 |
|---|---:|---:|
| 首次成功提高副本目标 | 49.666s | 34.189s |
| HTTP 200 成功率 | 71.410% | 79.068% |
| 541 秒副本占用 | 2206 Pod·s | 2378.5 Pod·s |
| 541 秒控制器查询 | 32 次 | 55.5 次 |

15 秒组观察到平均提前约 15.48 秒扩容、成功率高约 7.66 个百分点，同时副本
占用多约 7.82%、查询多约 73.44%。四次仍各丢弃一次迭代，全请求 p95 均约
10 秒，整体延迟没有改善到此前校准的服务标准。

![四次对照的收益与代价](assets/cadence-tradeoffs-20260907.png)

还有一项不能省略的结果：15 秒组首次看到 CPU 超阈值的信息也平均早了
8.51 秒。第一对中，信息早了 13.19 秒，后续等待只少了 1.97 秒；第二对中，
信息早了 3.83 秒，后续等待少了 11.97 秒。两对扩容都约早 15 秒，具体时间线
却并不相同。

所以“只主动改了一个配置”仍不等于“测到的所有差值都只能由它造成”。这次
匹配了代码和控制器偏移，但没有锁定底层采样相位，也无法消除共享主机波动。
若只报告扩容均值，我们会把输入数据更早可见的差异也算成减少协调等待的收益。
这正是保留原始时间链、重跑对照并独立复算的价值。

最终保留默认 **30 秒**，15 秒作为显式可选择、可复现的实验配置。它在本次
对照中呈现出有用的时间／服务与资源取舍，但每组两次、一个特意选中的启动偏移，
不足以决定全局默认值。完整逐次结果、配对差异和复核记录见
[协调周期对照报告](latency-cadence-followup-20260907.md)。本次没有追加参数搜索。

## 阅读与复现

- [实验协议与验收标准](latency-diagnostic.md)
- [六次时序诊断结果](latency-diagnostic-20260907.md)
- [四次协调周期对照结果](latency-cadence-followup-20260907.md)
